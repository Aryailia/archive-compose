...
const unitTests = function ($) {
  ...
  tape('Naive Method Tests', function (t) {
    ...
    t.deepEqual($.map((x, i, a) => (x % a.length + a.length) % a.length, $.range(-6,2)),
  ...
  tape('Reduce tests', function (t) {
    t.deepEqual(
      $($.range(0, 10, 2)).reduce((a, x) => a + x, 0).value(),
      [20],
      'range |> reduce sum');
    t.deepEqual(
      $($.range(0, 10, 2))
        .reduce((a, x) => { a.push(x); return a; }, []).value(),
      [[0,2,4,6,8]],
      'range |> reduce append ');
    t.deepEqual(
      $($.range(0, 20, 2))
